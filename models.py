...
class UNetGenerator(nn.Module):
    def __init__(self, in_channels=3, out_channels=3, num_filters=128):
        ...
        self.bottleneck = nn.Sequential(
            nn.Conv2d(num_filters * 8, num_filters * 8, kernel_size=3, padding=1),
            nn.InstanceNorm2d(num_filters * 8),
            nn.ReLU(inplace=True)
        )

        self.dec4 = self.decoder_block(num_filters * 16, num_filters * 4)
        self.dec3 = self.decoder_block(num_filters * 8, num_filters * 2)
        self.dec2 = self.decoder_block(num_filters * 4, num_filters)
